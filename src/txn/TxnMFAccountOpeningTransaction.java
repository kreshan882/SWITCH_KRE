...
public class TxnMFAccountOpeningTransaction {
	...
	public ISOMsg doProcessRequest(ISOMsg requestMsg,TxnKeyWords keyWD)throws Exception{
		...
		keyWD.setTXNNAME(TxnTypeCodes.MF_NAME_ACCOUNT_OPEN_TXN);
...
//			 if(! dbpr.getPerBatchLimitFromComponey(con, keyWD)){
//				 keyWD.setRESPONSECODE(ResponseCode.BATCH_LIMIT_NOTDEFINE);
//				 keyWD.setERROR_DESCRIPTION(ResponseCode.MSG_BATCH_LIMIT_NOTDEFINE) ;
//			 }
//		}
		if(! dbpr.getPerBatchLimitFromTerminalComponey(con, keyWD) ){
		...
		if(ResponseCode.SUCCESS.equals(keyWD.getRESPONSECODE())){
			...
			if(ResponseCode.SUCCESS.equals(keyWD.getRESPONSECODE())&& null != resMsg.getValue(63)){
				resMsg.set(54,resMsg.getString(61)); // Account No | Name
				TxnHelpMethods.substarctAccountNumber(resMsg.getString(61), keyWD);
				dbpr.updateTransactionAccountNo(con, keyWD);
			}
