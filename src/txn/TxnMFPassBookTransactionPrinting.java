...
public class TxnMFPassBookTransactionPrinting {
	...
	public ISOMsg doProcessRequest(ISOMsg requestMsg,TxnKeyWords keyWD)throws Exception{
		...
		keyWD.setTXNTYPE(TxnTypeCodes.MF_PASSBOOK_PRINTING);
		keyWD.setTXNNAME(TxnTypeCodes.MF_NAME_PASSBOOK_PRINTING);
		
		SecurityProcessing.decriptRequestData(keyWD);
		
		if(DBValidation.validate(dbpr, con, keyWD, false, true)){	
		...
		if(ResponseCode.SUCCESS.equals(keyWD.getRESPONSECODE())){
			...
			resMsg.unset(7);
//			resMsg.set(62,resMsg.getString(61));
			resMsg.set(61,resMsg.getString(59));
			resMsg.unset(59);
